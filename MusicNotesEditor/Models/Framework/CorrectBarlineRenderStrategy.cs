...
{
    public class CorrectBarlineRenderStrategy : MusicalSymbolRenderStrategy<Barline>
    {
        ...
        public CorrectBarlineRenderStrategy(IMeasurementService measurementService, IAlterationService alterationService, IScoreService scoreService) : base(scoreService)
        {
        ...
        public override void Render(Barline element, ScoreRendererBase renderer, FontProfile fontProfile)
        {
            ...
            Console.WriteLine($"RENDERING BARLINES: {measureWidth}");
            ...
            else if (element.RepeatSign == RepeatSignType.None)
            {
                ...
                if (scoreService.CurrentStaff != scoreService.CurrentScore.FirstStaff)
                {
                    ...
                    Console.WriteLine($"CORRESPONDING MEASURE {correspondingMeasure} AND {correspondingMeasure.BarlineLocationX}");
            ...
            if (element.RepeatSign == RepeatSignType.None)
            {
                ...
                if (!doNotDraw)
                {
                    ...
                    FieldInfo field = element.GetType().GetField("ActualRenderedBounds",
                        BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
                   
                    var newBounds = new Quadrangle(
                    ...
                    // For readonly fields, you need to use SetValue differently
                    field.SetValue(element, newBounds);
                }
        ...
        {
            if (scoreService.CurrentStaffNo == 1)
            {
                FieldInfo field = element.GetType().GetField(
                    "RenderedXPositionForFirstStaffInMultiStaffPart",
                    BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);

                if (field != null)
                {
                    field.SetValue(element, scoreService.CursorPositionX);
                }
            }

            FieldInfo field2 = element.GetType().GetField("ActualRenderedBounds",
                        BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
            ...
            // For readonly fields, you need to use SetValue differently
            field2.SetValue(element, newBounds);
        ...
        private bool IsLastBarline(Barline element)
        {
            return element == scoreService.CurrentStaff.Elements.OfType<Barline>().LastOrDefault()                          //Element is the last barline in the staff 
                && scoreService.CurrentStaff.Peek<NoteOrRest>(element, Manufaktura.Controls.Model.PeekStrategies.PeekType.NextElement) == null;  //and there are no notes or rests after it
        }
    }
}
